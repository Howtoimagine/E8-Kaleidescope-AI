...
class NarrativeStreamer:
    ...
    async def generate_and_add_entry(self, mind_state: 'E8Mind'):
        ...
        try:
            ...
            if len(significant_events) < 3: return # Guard clause was misplaced due to broken code
...
class E8Mind:
    ...
    def _snap_to_lattice(self, vector: np.ndarray, dim: int) -> np.ndarray:
        ...
        if kdtree is None:
            return vector # Fallback if no tree exists for this dimension
    ...
    async def _run_proactive_insight_synthesis(self):
        async with self.insight_cycle_lock:
            ...
            # --- FIX: Clean parent labels to prevent recursive prefixes ---
            ...
            new_entry = {
                "type": "insight_synthesis", "label": sanitize_line(f"Synthesis: {cleaned_a} + {cleaned_b}"), # Use the cleaned labels
    ...
    async def _async_call_llm_internal(self, prompt: str, **kwargs) -> str:
        ...
        tasks_to_gather = []
        if primary_task:
            tasks_to_gather.append(primary_task)
        if local_task:
            tasks_to_gather.append(local_task)
        
        results = await asyncio.gather(*tasks_to_gather, return_exceptions=True)
        
        primary_text = "[LLM ERROR] Primary task did not run."
        if primary_task:
             primary_text = results[0] if not isinstance(results[0], BaseException) else f"[LLM ERROR] {results[0]}"

        local_text = None
        if local_task:
            # The index of the local_task result depends on whether the primary_task was also gathered
            result_index = 1 if primary_task else 0
            
            # --- FIX START ---
            # Add a check to prevent IndexError if only the primary task ran
            if len(results) > result_index:
                local_text = results[result_index] if not isinstance(results[result_index], BaseException) else None
...
# === Embedded World-Model/HRL/Causal Patch ===
...
class StateVAEWorldModel:
    def __init__(self, input_dim: int, action_dim: int, latent_dim: int = 32, device: str = "cpu"):
        self.input_dim = int(input_dim); self.action_dim = int(action_dim); self.latent_dim = int(latent_dim)
        self.device = device
        self.max_buf = 20000
        self._s_buf = _np.zeros((self.max_buf, self.input_dim), dtype=_np.float32)
        self._a_buf = _np.zeros((self.max_buf, self.action_dim), dtype=_np.float32)
        self._ns_buf = _np.zeros((self.max_buf, self.input_dim), dtype=_np.float32)
        self._r_buf = _np.zeros((self.max_buf, 1), dtype=_np.float32)
        self._ptr = 0; self._size = 0
        self._ready_steps = 0
        if _TORCH:
            self._build_torch()
        else:
            rng = _np.random.default_rng(1337)
            self.K = rng.standard_normal((self.latent_dim, self.latent_dim)).astype(_np.float32)*0.03
            self.B = rng.standard_normal((self.action_dim, self.latent_dim)).astype(_np.float32)*0.03

    def _build_torch(self):
        D, Z, A = self.input_dim, self.latent_dim, self.action_dim
        class _VAE(nn.Module):
            def __init__(self, D, Z):
                super().__init__()
                self.e1 = nn.Linear(D, 128); self.e2 = nn.Linear(128, 128)
                self.mu  = nn.Linear(128, Z); self.lvg = nn.Linear(128, Z)
                self.d1 = nn.Linear(Z, 128); self.d2 = nn.Linear(128, 128); self.xo = nn.Linear(128, D)
            def encode(self, x):
                import torch
                h = torch.relu(self.e1(x)); h = torch.relu(self.e2(h))
                return self.mu(h), self.lvg(h)
            def decode(self, z):
                import torch
                h = torch.relu(self.d1(z)); h = torch.relu(self.d2(h)); return self.xo(h)
            def reparameterize(self, mu, logvar):
                import torch
                std = (0.5*logvar).exp(); eps = torch.randn_like(std); return mu + eps*std
            def forward(self, x):
                mu, logvar = self.encode(x); z = self.reparameterize(mu, logvar); xh = self.decode(z); return xh, mu, logvar, z
        class _Dyn(nn.Module):
            def __init__(self, Z, A):
                super().__init__()
                import torch, torch.nn as nn
                self.K = nn.Parameter(torch.randn(Z, Z)*0.03); self.B = nn.Parameter(torch.randn(A, Z)*0.03)
                self.r = nn.Sequential(nn.Linear(Z, 64), nn.ReLU(), nn.Linear(64, 1))
            def step(self, z, a):
                import torch
                return torch.relu(z @ self.K + a @ self.B)
            def reward(self, z): return self.r(z)
        import torch
        self.vae = _VAE(D, Z).to(self.device); self.dyn = _Dyn(Z, A).to(self.device)
        self.opt_vae = torch.optim.Adam(self.vae.parameters(), lr=1e-3)
        self.opt_dyn = torch.optim.Adam(self.dyn.parameters(), lr=1e-3)

    def is_ready(self) -> bool: return self._ready_steps > 256

    def encode(self, x) -> _np.ndarray:
        if _TORCH:
            import torch
            with torch.no_grad():
        ...
        return self._np_encode(_np.asarray(x, dtype=_np.float32))

    def _np_encode(self, x: _np.ndarray) -> _np.ndarray:
        if not hasattr(self, "_E"):
            rng = _np.random.default_rng(13)
            self._E = rng.standard_normal((self.input_dim, self.latent_dim)).astype(_np.float32) * 0.1
            self._D = rng.standard_normal((self.latent_dim, self.input_dim)).astype(_np.float32) * 0.1
        return x @ self._E
    def _np_decode(self, z: _np.ndarray) -> _np.ndarray: return z @ self._D

    def imagine_with_policy(self, state, policy_like, horizon: int = 5):
        s = _np.asarray(state, dtype=_np.float32); z = self.encode(s); tot = 0.0
        for t in range(max(1,int(horizon))):
            a = policy_like.select_action(s, deterministic=False) if hasattr(policy_like, "select_action") else _np.zeros((self.action_dim,), dtype=_np.float32)
            if _TORCH:
                import torch
                with torch.no_grad():
                    z_t = torch.tensor(z[None,:], dtype=torch.float32); a_t = torch.tensor(a[None,:], dtype=torch.float32)
                    z = self.dyn.step(z_t, a_t).squeeze(0).cpu().numpy().astype("float32")
                    r = float(self.dyn.reward(torch.tensor(z[None,:], dtype=torch.float32)).item())
            else:
                z = _np.maximum(0.0, z @ self.K + a @ self.B); r = float(z.mean())
            tot += r
        return {"z_mean": float(_np.mean(z)), "pred_return": float(tot)}

    def observe(self, s, a, ns, r):
        i = self._ptr; self._s_buf[i] = s; self._a_buf[i] = a; self._ns_buf[i] = ns; self._r_buf[i,0] = float(r)
        self._ptr = (self._ptr + 1) % self.max_buf; self._size = min(self._size + 1, self.max_buf); self._ready_steps += 1
        self.train_step(32)

    def train_step(self, batch: int = 64):
        if self._size < batch: return
        idx = _np.random.randint(0, self._size, size=batch)
        S = self._s_buf[idx]; A = self._a_buf[idx]; NS = self._ns_buf[idx]; R = self._r_buf[idx,0:1]
        if _TORCH:
            import torch
            x = torch.tensor(S, dtype=torch.float32)
            xh, mu, logvar, z = self.vae(x)
            recon = torch.mean((xh - x)**2)
            kld = -0.5 * torch.mean(1 + logvar - mu.pow(2) - logvar.exp())
            z1 = self.dyn.step(z, torch.tensor(A, dtype=torch.float32))
            with torch.no_grad(): mu_ns, _ = self.vae.encode(torch.tensor(NS, dtype=torch.float32))
            dyn = torch.mean((z1 - mu_ns)**2)
            r_hat = self.dyn.reward(z)
            rew = torch.mean((r_hat - torch.tensor(R, dtype=torch.float32))**2)
            loss_vae = recon + 0.05 * kld; loss_dyn = dyn + 0.5 * rew
            self.opt_vae.zero_grad(); loss_vae.backward(); self.opt_vae.step()
            self.opt_dyn.zero_grad(); loss_dyn.backward(); self.opt_dyn.step()
        else:
            Z = self._np_encode(S); ZN = self._np_encode(NS)
            X = _np.concatenate([Z, A], axis=1); lam = 1e-3
            XtX = X.T @ X + lam * _np.eye(X.shape[1], dtype=_np.float32); XtY = X.T @ ZN
            W = _np.linalg.pinv(XtX) @ XtY
            self.K = W[:self.latent_dim, :].astype(_np.float32); self.B = W[self.latent_dim:, :].astype(_np.float32)

class CausalEngine:
    def __init__(self, console=None, max_hist: int = 20000):
        self.console = console; self.max_hist = int(max_hist)
        self.vars = ["bh_pressure","coherence","entropy","intensity","novelty","reward","action_norm"]
        self.H = {k: [] for k in self.vars}; self.graph = {k:{} for k in self.vars}
    def update_on_step(self, mind, action, reward):
        try:
            self._push("bh_pressure", float(getattr(mind, "black_hole_pressure", 0.0)))
            mv = getattr(mind.mood, "mood_vector", {})
            self._push("coherence", float(mv.get("coherence", 0.0)))
            self._push("entropy", float(mv.get("entropy", 0.0)))
            self._push("intensity", float(mv.get("intensity", 0.0)))
            self._push("novelty", float(mv.get("novelty", 0.0) if "novelty" in mv else mv.get("valence", 0.0)))
            self._push("reward", float(reward))
            self._push("action_norm", float((_np.linalg.norm(action) if action is not None else 0.0)))
        except Exception: pass
    def on_event(self, kind: str, mind):
        try:
            self._relearn_graph()
            if self.console: self.console.log(f"[Causal] Refit after event '{kind}'. Top: {self.top_edges(5)}")
        except Exception as e:
            if self.console: self.console.log(f"[Causal] Fit error: {e}")
    def _push(self, k,v):
        H = self.H[k]; H.append(float(v)); 
        if len(H) > self.max_hist: del H[:len(H)-self.max_hist]
    def _relearn_graph(self):
        import numpy as _np2
        for tgt in self.vars:
            y = _np2.asarray(self.H[tgt], dtype=_np2.float32)
            if y.size < 64: continue
            Xcols = []; names = []
            for src in self.vars:
                if src == tgt: continue
                x = _np2.asarray(self.H[src], dtype=_np2.float32)
                n = min(x.size, y.size); x = x[-n:]; y1 = y[-n:]
                Xcols.append(x[:-1]); names.append(src)
            if not Xcols: continue
            X = _np2.stack(Xcols, axis=1); y2 = y1[1:]
            X = _np2.concatenate([X, _np2.ones((X.shape[0],1), dtype=_np2.float32)], axis=1)
            try:
                w, *_ = _np2.linalg.lstsq(X, y2, rcond=None)
            except Exception: continue
            coefs, edges = w[:-1], {}
            for name, c in zip(names, coefs):
                v = float(c)
                if abs(v) > 0.02: edges[name] = v
            self.graph[tgt] = edges
    def top_edges(self, k=5):
        pairs = []
        for tgt, inc in self.graph.items():
            for src, w in inc.items():
                pairs.append((abs(w), f"{src} -> {tgt} ({w:+.3f})"))
        pairs.sort(reverse=True); return [s for _, s in pairs[:k]]

@dataclass
class _ManagerDirective:
    name: str
    weight: float = 0.35
    goal_vec: Optional[_np.ndarray] = None

class _NullPE:
    def cosine(self, a,b):
        a=_np.asarray(a,dtype=_np.float32); b=_np.asarray(b,dtype=_np.float32)
        n=float(_np.linalg.norm(a)*_np.linalg.norm(b)+1e-9); 
        return float((_np.dot(a,b))/n)

class HierarchicalController:
    def __init__(self, goal_field, potential_evaluator, console=None, update_every:int=50):
        self.goal_field = goal_field; self.potential_evaluator = potential_evaluator
        self.console = console; self.update_every = int(update_every)
        self.step = 0; self.current: Optional[_ManagerDirective] = None
    def _pick_goal(self) -> _ManagerDirective:
        try:
            if not getattr(self.goal_field, "goals", None): return _ManagerDirective("stability",0.2,None)
            g = max(self.goal_field.goals.items(), key=lambda kv: kv[1].get("activation", 0.0))
            name, data = g
            import numpy as _np3
            return _ManagerDirective(name=name, weight=0.35, goal_vec=_np3.asarray(data.get("embedding"), dtype=_np3.float32) if data.get("embedding") is not None else None)
        except Exception:
            return _ManagerDirective("stability",0.2,None)
    def maybe_update(self, step:int):
        self.step = step
        if (step % self.update_every)==0 or self.current is None:
            self.current = self._pick_goal()
            if self.console: self.console.log(f"[HRL] Manager directive -> {self.current.name} (w={self.current.weight:.2f})")
    def shape_reward(self, state, next_state, base_reward: float) -> float:
        if self.current is None: return float(base_reward)
        shaped = float(base_reward)
        try:
            if self.current.goal_vec is not None and hasattr(self.potential_evaluator, "cosine"):
                before = float(self.potential_evaluator.cosine(state, self.current.goal_vec))
                after  = float(self.potential_evaluator.cosine(next_state, self.current.goal_vec))
