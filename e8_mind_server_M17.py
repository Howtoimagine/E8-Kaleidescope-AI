# >>> AUTO-APPLIED M17 RUNTIME PATCHES <<<
try:
    from m17_runtime_patches import apply_patches as _m17_apply
    _m17_apply(globals())
...
#
# --- M17 UPGRADE NOTES ---
# This version incorporates a series of state-of-the-art upgrades based on
# a provided research plan. Each implementation is marked with a comment,
# e.g., [UPGRADE 1], corresponding to the plan's points.
...
# Misc
SEMANTIC_DOMAIN = "Quantum Cognitive Entanglement"
...
DATA_SOURCES: Dict[str, Any] = {
    "arxiv_cs_cl": {
        "type": "arxiv_api",
        "url": "http://export.arxiv.org/api/query?search_query=cat:cs.CL&sortBy=submittedDate&sortOrder=descending&max_results=10",
...
try:
    ...
    class _FallbackPrompts:
        def render(self, key, **vars):
            ...
            return f"{persona}\n\n{domain_hint}\n\n{q}"
    class _FallbackSem:
        name = "default"; base_domain = SEMANTIC_DOMAIN
        def persona_prefix(self, mood):
                # Safely get mood values, defaulting to 0.5 (neutral)
                intensity = mood.get('intensity', 0.5)
                entropy = mood.get('entropy', 0.5)
                coherence = mood.get('coherence', 0.5)

                # Return a descriptive persona based on the mood state
        ...
        def pre_embed(self, t):
            # Prefix the text with the base domain to add context.
            # This helps the embedding model create more relevant vectors.
            if self.base_domain and isinstance(t, str):
                return f"{self.base_domain}: {t}"
            return t
        def post_embed(self, v):
            # Normalize the vector to ensure it has a unit length.
            # This is critical for accurate cosine similarity calculations.
            if v is not None and hasattr(v, '__abs__'):
                norm = np.linalg.norm(v)
                if norm > 1e-9:  # Avoid division by zero
                    return v / norm
            return v
        def rerank(self, c):
            # With a 10% chance, swap the top two results to introduce novelty.
            # This helps prevent the AI from becoming too predictable.
            if random.random() < 0.1 and isinstance(c, list) and len(c) > 1:
                c[0], c[1] = c[1], c[0]  # Swap the first two items
            return c
...
class MoodEngine:
    def __init__(self, console: Console, baseline=0.5, decay_rate=0.995):
        ...
        self._wx_repeat = 0
        if self.console:  # <-- ADD THIS CHECK
...
class KanervaSDM:
    ...
    def write(self, vec_embed):
        ...
        if hasattr(self.kdtree._impl, 'query_radius'):
            indices = self.kdtree._impl.query_radius(vec8d.reshape(1, -1), r=self.radius)[0]
            if len(indices) > 0:
...
class MemoryManager:
    # Replace the MemoryManager constructor with this version.
    def __init__(self, embedding_fn, mood, subconscious, run_id, probe, llm_caller, mind_instance, **kwargs):
        ...
        # [UPGRADE 1] Hopfield Network
        self.hopfield = HopfieldModern(self)
        # [UPGRADE 2] Kanerva SDM
        self.sdm = KanervaSDM(mind_instance)
        # [UPGRADE 3] Vector-Symbolic Architecture
        self.vsa = VSA(EMBED_DIM)
        # --- BUG FIX: Instantiate the MicroReranker ---
    ...
    def find_similar_in_main_storage(self, query_vector: np.ndarray, k: int = 5) -> List[tuple[str, float]]:
        """Upgraded retrieval method with regularization and community bias."""
        if self.main_kdtree is None or not self._main_storage_ids:
            return []
        
        # [UPGRADE 4 & 6] Increase initial search pool for better reranking
        initial_k = k * 4
        initial_k = min(initial_k, len(self._main_storage_ids))
        if initial_k == 0: return []
        
        distances, indices = self.main_kdtree.query(query_vector, k=initial_k)
        
        # Ensure results are iterable
        distances = np.atleast_1d(distances)
        indices = np.atleast_1d(indices)

        # [UPGRADE 4 & 6] Rerank candidates with new scoring
        reranked_candidates = []
        query_node_community = -1 # Placeholder for query community
        for dist, idx in zip(distances, indices):
            ...
            # [UPGRADE 6] Hubness Penalty
            degree = self.graph_db.graph.degree(nid)
            hub_penalty = max(0, degree - 12) * 0.005 # Small penalty for high connectivity
            score += hub_penalty

            # [UPGRADE 2] Kanerva SDM Gating
            sdm_strength = self.sdm.read_strength(self.main_vectors[nid])
            # Penalize weak memories by increasing their distance score
            score *= (1.2 - sdm_strength)

            # [UPGRADE 4] Community Bias
            node_community = node_data.get("community_id", -2)
            if query_node_community != -1 and node_community == query_node_community:
                score *= 0.95 # 5% score reduction (bonus) for same community
    ...
    async def add_entry(self, entry_data: dict, parent_ids: Optional[List[str]] = None, target_shells: Optional[List[int]] = None, precomputed_vec: Optional[np.ndarray] = None) -> str:
        ...
        if pre_vec is not None:
            ...
            # [UPGRADE 2] Write to Kanerva SDM
            self.sdm.write(pre_vec)
    ...
    def collect_cluster(self, center_id: str, radius: int = 4) -> List[str]:
        """
        Collects a cluster of nodes around a center using DBSCAN on their embeddings.
        Falls back to a simple ego graph if dependencies are missing or the cluster is too small.
        """
        if DBSCAN is None:
        ...
        # Get vectors and filter out nodes without one
        vectors = [self.main_vectors.get(nid) for nid in nodes_in_radius]
        valid_nodes_indices = [i for i, v in enumerate(vectors) if v is not None and np.any(v)]
        
        if len(valid_nodes_indices) < 3:
            return [nodes_in_radius[i] for i in valid_nodes_indices]

        valid_vector_matrix = np.array([vectors[i] for i in valid_nodes_indices])
        valid_node_ids = [nodes_in_radius[i] for i in valid_nodes_indices]

        # Perform DBSCAN clustering
        clustering = DBSCAN(eps=0.85, min_samples=2, metric='cosine').fit(valid_vector_matrix)
        
        # Find the cluster label for the center node
        try:
            center_idx_in_valid = valid_node_ids.index(center_id)
        except ValueError:
            return [center_id] # Center node had no vector, return it alone

        center_cluster_label = clustering.labels_[center_idx_in_valid]
        if center_cluster_label == -1:
            return [center_id] # Center node is considered noise

        # Return all node IDs belonging to the same cluster as the center
    ...
    async def synthesize_remnant(self, cluster_nodes: List[str], label_hint: str, is_macro: bool = False) -> Tuple[Optional[Dict], Optional[np.ndarray], float]:
        ...
        # Create a weighted average of the vectors based on node temperature
        # FIX: The original logic for getting weights was buggy. Corrected to iterate cluster_nodes.
        weights = np.array([
            (self.graph_db.get_node(nid) or {}).get('temperature', 1.0)
            for nid in cluster_nodes if nid in self.main_vectors
        ])
        if weights.sum() < 1e-9: weights = np.ones(len(cluster_vectors))
        weights /= weights.sum()
        
        remnant_vec = np.average(np.array(cluster_vectors), axis=0, weights=weights)

        # [UPGRADE 1] Hopfield clean-up pass on the synthesized remnant vector
        if hasattr(self, 'hopfield') and self.hopfield.prototypes.shape[1] > 0:
            remnant_vec = self.hopfield.clean_up(remnant_vec)

        # Use LLM to generate a label and metaphor for the new remnant
    ...
    async def snapshot(self):
        ...
        async with self.lock:
            ...
            self._rebuild_main_kdtree() # Ensure tree is up-to-date
    ...
    async def mutate_memory(self, k: int = 1):
        """Creates novel variations of existing high-potential concepts."""
        ...
        for new_text in new_metaphors:
            ...
            new_entries.append({
                "type": "mutation", "label": sanitize_line(f"mutation of {seeds[0][2].get('label')}"),
                "metaphor": new_text, "rating": new_rating, "embedding": new_emb,
                "temperature": 0.2, "step": self.mind.step_num
            })

        new_node_ids = [await self.add_entry(entry, parent_ids=parent_ids) for entry in new_entries]
        
        self.mind.console.print(Panel(
            f"Generated {len(new_node_ids)} new mutated concepts (Entropy Boost: {temp_boost:.2f}x).",
            title="[bold yellow]MUTATION[/bold yellow]", border_style="yellow"
        ))
        return new_node_ids

    def _node_coh(self, nid: str) -> float:
        try:
            return float(self.graph_db.graph.nodes[nid].get("coherence", 0.5))
        except Exception:
            return 0.5

    def _trim_degree(self, nid: str, max_deg: int = 8):
        G = self.graph_db.graph
        try:
            if not G.has_node(nid): return
            deg = list(G[nid].items())
            if len(deg) <= max_deg: return
            scored = []
            for nb, edata in deg:
                mc = 0.5 * (self._node_coh(nid) + self._node_coh(nb))
                scored.append((mc, nb))
            scored.sort()
            to_drop = [nb for _, nb in scored[:max(0, len(deg)-max_deg)]]
            for nb in to_drop:
                try: G.remove_edge(nid, nb)
                except Exception: pass
        except Exception:
            pass

    def _allow_edge(self, parent_id: str, child_id: str) -> bool:
        mean_coh = 0.5 * (self._node_coh(parent_id) + self._node_coh(child_id))
        if mean_coh < 0.55:
            return False
        try:
            p_last = self.graph_db.graph.nodes[parent_id].get("last_step", -10**9)
            c_last = self.graph_db.graph.nodes[child_id].get("last_step", -10**9)
            return (p_last > self.mind.step_num - 200) and (c_last > self.mind.step_num - 200)
        except Exception:
            return True

    def _ema(self, old: float, x: float, a: float = 0.95) -> float:
        return float(a*old + (1.0-a)*x)

    def evict_if_needed(self, K: int = 50000):
        G = self.graph_db.graph
        try:
            if G.number_of_nodes() <= K: return
            for nid, d in G.nodes(data=True):
                r = float(d.get("rating", 0.5))
                d["reward_ma"] = self._ema(float(d.get("reward_ma", r)), r)
            degs = sorted(((G.degree(n), n) for n in G.nodes()), reverse=True)
            protect = set(n for _, n in degs[:max(1, len(degs)//10)])
            victims = sorted(((float(G.nodes[n].get("reward_ma", 0.5)), n) for n in G.nodes() if n not in protect))[:max(1, int(0.05*G.number_of_nodes()))]
            for _, n in victims:
                try:
                    G.remove_node(n)
                    self.main_vectors.pop(n, None)
                except Exception:
                    pass
    ...
    def merge_redundant_lowcoh(self, cos_thresh: float = 0.985, max_checks: int = 2000):
        G = self.graph_db.graph
        try:
            ids = list(self.main_vectors.keys())[:max_checks]
            if len(ids) < 2: return
            vecs = np.vstack([self.main_vectors[i] for i in ids])
            # normalize
            ...
            for i in range(len(ids)):
                for j in range(i+1, len(ids)):
                    if sims[i, j] >= cos_thresh:
                        a, b = ids[i], ids[j]
                        mc = 0.5*(self._node_coh(a) + self._node_coh(b))
                        if mc < 0.55:
                            keep, drop = (a, b) if self._node_coh(a) >= self._node_coh(b) else (b, a)
                            try:
                                for nb in list(G.neighbors(drop)):
                                    if nb != keep:
                                        G.add_edge(keep, nb, **G[drop][nb])
                                G.remove_node(drop)
                                self.main_vectors.pop(drop, None)
                            except Exception:
                                pass
            self._rebuild_main_kdtree()
        except Exception:
    ...
    def find_similar_with_hub_penalty(self, query_vector: np.ndarray, k: int = 20, lam: float = 0.05, kappa: float = 0.02):
        """
        Finds similar nodes, applying a penalty for highly-connected "hub" nodes
        and a bonus for nodes in well-distributed, non-collapsed neighborhoods.
        """
        # [UPGRADE 6] Increased base k to provide a better pool for regularization
        base = self.find_similar_in_main_storage(query_vector, k=max(k * 2, 40))
        if not base: return base
        G = self.graph_db.graph
        scored = []

        for nid, dist in base:
            node_data = self.graph_db.get_node(nid)
            if not node_data: continue

            # [UPGRADE 6] Hubness penalty for over-connected nodes
            deg = G.degree(nid) if G.has_node(nid) else 0
            hub_penalty = max(0, deg - 8) * lam
            
            # [UPGRADE 6] Local curvature bonus for well-separated neighborhoods
            # This is calculated on-the-fly and can be slow. A future optimization
            # would be to pre-calculate and cache this on nodes periodically.
            curvature_bonus = 0.0
            node_vec = self.main_vectors.get(nid)
            if node_vec is not None:
                neighbors = self.find_similar_in_main_storage(node_vec, k=8)
                if len(neighbors) > 2:
                    # Distances of the candidate's neighbors to the candidate itself
                    neighbor_dists = [d for _, d in neighbors[1:]] # Exclude self-distance
                    if len(neighbor_dists) > 1:
                        # Reward higher std dev (less collapsed neighborhood) with a score reduction
                        curvature_bonus = np.std(neighbor_dists) * kappa

            # Combine scores: lower is better. Penalties are added, bonuses subtracted.
            adj_dist = float(dist + hub_penalty - curvature_bonus)
            scored.append((nid, adj_dist))
...
class DataIngestionPipeline:
    ...
    async def _process_arxiv(self, name: str, config: Dict[str, Any]):
        ...
        async with aiohttp.ClientSession() as session:
            # Use the configured URL from the DATA_SOURCES dictionary
...
class MetricsManager:
    """A placeholder for the missing MetricsManager class."""
    def __init__(self, run_id):
        self.run_id = run_id
    ...
    def log(self, event_type: str, data: dict):
        ...
        try:
            ...
            # Use console if available, otherwise print
            msg = f"[MetricsManager] Failed to log: {e}"
...
class ContextBandit:
    """A placeholder for the missing ContextBandit class."""
    def __init__(self, arms: list, path_json: str):
        self.arms = arms
        self.path = path_json
    ...
    def load(self):
        ...
        if data and 'counts' in data and 'values' in data:
            self.counts = data['counts']
            self.values = data['values']

    def save(self):
        safe_json_write(self.path, {'counts': self.counts, 'values': self.values})
    ...
    def pull(self):
        ...
        ucb_values = [
            val + math.sqrt(2 * math.log(total_counts) / (count + 1e-6))
    ...
    def update(self, arm_index, reward):
        ...
        old_val = self.values[arm_index]
        self.values[arm_index] = ((n - 1) / n) * old_val + (1 / n) * reward
        self.save()
async def handle_get_metrics_summary(request):
    """Placeholder for the /metrics/summary endpoint."""
    return web.json_response({"status": "ok", "message": "Metrics endpoint is active."})

async def handle_post_quantizer(request):
    """Placeholder for the /quantizer endpoint."""
    ...
    try:
        ...
        return web.json_response({"error": "Missing 'type' in request"}, status=400)
    except Exception as e:
        return web.json_response({"error": str(e)}, status=500)

async def handle_post_snapshot(request):
    """Placeholder for the /snapshot endpoint."""
    mind = request.app['mind']
    asyncio.create_task(mind.memory.snapshot())
    return web.json_response({"status": "ok", "message": "Snapshot creation initiated."})
    
def bump_temps(memory, node_ids, amount=0.6):
    """Placeholder for the missing bump_temps function."""
    for nid in node_ids:
        memory.spike_temperature(nid, amount)

def attach_to_mind(mind):
    """Attaches additional modules to the mind instance post-initialization."""
    ...
    if getattr(mind, "world_model", None) is None:
        ...
        def _wm_lazy_init(n):
            try:
                wm = StateVAEWorldModel(input_dim=int(n), action_dim=int(mind.action_dim))
            except Exception:
                wm = None
            setattr(mind, "world_model", wm)
        setattr(mind, "_wm_lazy_init", _wm_lazy_init)
    if getattr(mind, "causal", None) is None:
        mind.causal = CausalEngine(getattr(mind, "console", None))
    if getattr(mind, "hrl", None) is None:
        pe = getattr(mind, "potential_evaluator", None) or _NullPE()
        mind.hrl = HierarchicalController(getattr(mind, "goal_field", None), pe, getattr(mind, "console", None))
...
class E8Mind:
    ...
    def __init__(self, semantic_domain_val, run_id, llm_client_instance, client_model, embedding_model_name, embed_adapter, embed_in_dim, console: Console):
        self.metrics = MetricsManager(getattr(self, 'run_id', 'run'))
        self._quantizer_override = None  # can be set via POST /quantizer
        ...
        self.bandit = ContextBandit(
            ...
            path_json=get_path("bandit_state.json", getattr(self, "run_id", "run")),
        )
        ...
        self.state_dim = self._safe_compute_state_dim()  # Mood + Goals + Attention + Dynamics
    ...
    def _snap_to_lattice(self, vector: np.ndarray, dim: int) -> np.ndarray:
        ...
        try:
            ...
            # This now correctly returns a 1D vector from the correct attribute.
    ...
    async def _run_proactive_insight_synthesis(self):
        async with self.insight_cycle_lock:
            ...
            new_entry = {
                ...
                # CORRECTED: Uses the cleaned labels for the new entry.
            ...
            # [UPGRADE 1, 3, 7] Pre-storage processing pipeline
            ...
            # M17: enforce parent→child edge gating and degree caps
            try:
            ...
            if final_rating > 0.8: asyncio.create_task(self.validator.validate_insight(new_node_id))
    ...
    async def _project_self_into_memory(self):
        ...
        """
        if os.getenv('E8_ALLOW_CODE_INGEST', '0') != '1':
            self.console.log('[SelfProject] Skipped by E8_ALLOW_CODE_INGEST=0')
            return
        try:
    ...
    async def run_cognitive_cycle(self, max_steps=99200, mode='quantum'):
        ...
        with Progress("[progress.description]{task.description}", BarColumn(), "[progress.percentage]{task.percentage:>3.0f}%",
            ...
            for step in range(max_steps):
                ...
                # WM init + HRL tick
                try:
                    if getattr(self, "_wm_lazy_init", None) and getattr(self, "world_model", None) is None and isinstance(current_state, np.ndarray):
                        self._wm_lazy_init(current_state.size)
                except Exception:
                    pass
                try:
                    if getattr(self, "hrl", None):
                ...
                # Latent warm-up planning
                try:
                    if getattr(self, "world_model", None) and self.world_model.is_ready():
                        _ = self.world_model.imagine_with_policy(current_state, self.agent, horizon=5)
                except Exception:
                    pass
                ...
                try:
                    if getattr(self, "hrl", None):
                        reward = self.hrl.shape_reward(current_state, next_state, base_reward)
                except Exception:
                    pass
                try:
                    if getattr(self, "causal", None):
                        self.causal.update_on_step(self, clamped_action, reward)
                except Exception:
                    pass
                
                # 6. Store experience and train the RL agent.
                if self.agent:
                    # The order here is crucial: (s_t, a_t, r_t, s_{t+1}, done)
                    try:
                        if getattr(self, "world_model", None) and isinstance(current_state, np.ndarray) and isinstance(next_state, np.ndarray):
    ...
    async def _async_call_llm_internal(self, prompt: str, **kwargs) -> str:
        ...
        try:
            # FIX: Access self correctly, not self.mind
        ...
        if local_task:
            ...
            # --- BUG FIX: Check if the result index exists before accessing it ---
...
async def handle_add_concept(request):
    ...
    try:
        ...
        # Guard: optional block of code-like text if E8_ALLOW_CODE_INGEST=0
        if os.getenv('E8_ALLOW_CODE_INGEST', '1') != '1' and looks_like_code(text):
            return web.json_response({'error': 'Code ingestion disabled by policy'}, status=400)
...
# These are moved here from main() to be accessible globally.
llm_client = None
model_name = "stub"
...
def looks_like_code(text: str) -> bool:
    """Heuristic to detect if a text blob is source code."""
    ...
    indicators = [
        ...
        "#include <", "#!/usr", "using System", "<html", "</html", "<?php"
    ]
    hits = sum(1 for s in indicators if s in text)
    colon_lines = sum(1 for line in text.splitlines() if line.strip().endswith(":"))
    brace_rate = sum(ch in text for ch in "{};") / max(1, len(text))
    return hits >= 2 or colon_lines >= 3 or brace_rate > 0.04
